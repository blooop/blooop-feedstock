#!/usr/bin/env python3
"""
Update script for pkl package in the blooop-feedstock.

This script monitors the apple/pkl GitHub repository for new releases
...
RECIPE_PATH = Path("recipes/pkl/recipe.yaml")

# Platform-specific binary names
PLATFORM_BINARIES = {
    "linux-64": "pkl-linux-amd64",
    "linux-aarch64": "pkl-linux-aarch64",
    "osx-64": "pkl-macos-amd64",
    "osx-arm64": "pkl-macos-aarch64",
    "win-64": "pkl-windows-amd64.exe",
}
...
def get_latest_release():
    ...
    try:
        with urllib.request.urlopen(GITHUB_API_URL) as response:
            ...
            # pkl uses bare version numbers (e.g., "0.30.2"), no v prefix
...
def get_version(version_arg: str | None = None):
    """Get pkl version - either from argument or fetch latest from GitHub."""
    if version_arg:
...
def update_recipe(version: str, checksums: dict[str, str]) -> None:
    ...
    # Update version in all URLs (pkl uses bare version, no v prefix)
    if current_version:
        recipe = re.sub(
            rf'(https://github.com/{GITHUB_REPO}/releases/download/){current_version}(/)',
            rf'\g<1>{version}\g<2>',
            recipe
        )
...
def main():
    """Main function to update pkl recipe."""
    print("Updating pkl package...")
    ...
    print("pkl package update complete!")
